...
- [x] T001 Create migrations/013_create_email_change_requests.sql per data-model.md and apply to cflag_dmr_dev
- [x] T002 Create migrations/014_create_callsign_update_requests.sql per data-model.md and apply to cflag_dmr_dev
- [x] T003 Create app/profile/ directory with stub files: manager.php, email_change.php, callsign.php
...
- [x] T004 Implement app/profile/manager.php: get_profile(), update_display_name(), change_password() per contracts/profile.md
- [x] T005 [P] Implement app/profile/email_change.php: request_email_change(), confirm_email_change(), get_pending_email_change() per contracts/profile.md
- [x] T006 [P] Implement app/profile/callsign.php: submit_callsign_request(), get_user_callsign_requests(), get_pending_callsign_requests(), approve_callsign_request(), deny_callsign_request(), get_open_callsign_request() per contracts/profile.md
...
- [x] T007 [US1] Create public/user/profile.php — require_login(); show current display name, username, email, callsign, account status, registration date; display name update form (POST action=update_name); password change form (POST action=change_password, fields: current_password, new_password, confirm_password); CSRF on all forms; flash messages for success/error
- [x] T008 [US1] Add "My Profile" link to public/index.php (unified dashboard) for all authenticated users
...
- [x] T009 [US2] Add email change section to public/user/profile.php — shows current email; POST action=request_email_change with new_email field; if pending request exists, show pending notice with new_email and option to cancel; CSRF
- [x] T010 [US2] Create public/verify-email-change.php — accepts ?token= GET parameter; calls confirm_email_change($token); redirects to /user/profile.php with success flash or shows error for expired/invalid tokens
...
- [x] T011 [US3] Add callsign update section to public/user/profile.php — shows current callsign; if no open request: show request form (new_callsign, explanation fields, POST action=request_callsign); if open request: show pending status with requested callsign; if past requests: show status history table; CSRF
- [x] T012 [US3] Create public/admin/users/callsign-requests.php — list all pending callsign_update_requests; POST action=approve (with notes) and action=deny (with notes); CSRF; links back to admin/users/
- [x] T013 [US3] Add "Callsign Requests" link with pending count badge to public/admin/users/index.php (if pending requests > 0)
...
- [x] T014 [US4] Extend public/admin/users/view.php with devices table and moderation history sections (view.php IS the admin profile page) — require_role('system_admin'); accept GET id; fetch user record, roles, devices, mod_log entries; render full account detail: display name, username, email, callsign, DMR ID, roles, registration date, moderation state, devices table, moderation history table; link back to /admin/users/
- [x] T015 [US4] Add "View Profile →" link to each user row in public/admin/users/index.php (username already links to view.php; confirmed)
