...
- [ ] T001 Create `public/admin/users/` directory (will hold index.php and view.php)
...
- [ ] T010 [US1] Update `public/admin/index.php` — replace `require_admin()` call with `require_role('admin')`; ensure `app/auth/roles.php` is included (add `require_once` at top alongside existing includes); verify page still renders correctly for the migrated system_admin account
...
- [ ] T017 [US3] Register cron job for expire-mutes script — add to root crontab via `crontab -e`: `* * * * * php /opt/cflag-dmr/scripts/expire-mutes.php >> /var/log/cflag-dmr-cron.log 2>&1`; create `/var/log/cflag-dmr-cron.log` with `touch` if it doesn't exist; verify cron is registered with `crontab -l`
...
- [ ] T019 Add "User Management" navigation link to `public/admin/index.php` pointing to `/admin/users/` — shown only to users with `system_admin` role (use `user_has_role()` to gate visibility); mobile-friendly nav link styling per existing `.nav-link` class

- [ ] T020 [P] Audit all new and modified `public/` files for output escaping — verify every variable rendered to HTML passes through `htmlspecialchars($val, ENT_QUOTES, 'UTF-8')`; check: `public/admin/users/index.php`, `public/admin/users/view.php`, `public/admin/index.php`; fix any unescaped output found

- [ ] T021 [P] Verify mobile-first CSS on `public/admin/users/index.php` and `public/admin/users/view.php` — test in browser at 375px viewport width; confirm table uses `overflow-x: auto` wrapper; confirm all buttons meet 44px minimum touch target (Principle VII); add any missing CSS to `public/assets/css/app.css`

- [ ] T022 Run all 8 manual test scenarios from `specs/003-role-permissions/quickstart.md` — confirm each passes; document any failures and fix before marking feature complete
