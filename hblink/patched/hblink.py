...
'''
Patched hblink.py: per-device passphrase authentication via peer_auth.json.

The RPTK handler (challenge-response login) looks up each peer's passphrase
from /hblink3/json/peer_auth.json instead of using the shared MASTER passphrase.
PHP writes peer_auth.json whenever a device is approved or denied; the cache
refreshes on every login attempt via mtime comparison so updates are near-live.
All other behaviour is identical to the upstream hblink.py.
'''
...
PEER_AUTH_JSON = '/hblink3/json/peer_auth.json'
...
def _reload_peer_auth():
    global _peer_auth
    try:
        mtime = _os.path.getmtime(PEER_AUTH_JSON)
        if mtime <= _peer_auth['_mtime']:
            return
        with open(PEER_AUTH_JSON, 'r') as _f:
            data = _json.load(_f)
        _peer_auth['peers']   = data.get('peers', {})
        _peer_auth['version'] = data.get('version', -1)
        _peer_auth['_mtime']  = mtime
        logger.info('(PEER_AUTH) Reloaded peer_auth.json: %d peers, version %d',
                    len(_peer_auth['peers']), _peer_auth['version'])
    except Exception as _e:
        logger.warning('(PEER_AUTH) Could not reload peer_auth.json: %s', _e)


def _peer_passphrase(peer_id_int):
    '''Return the passphrase bytes for peer_id_int, or None if not allowed.'''
    _reload_peer_auth()
    entry = _peer_auth['peers'].get(str(peer_id_int))
    if entry and entry.get('allowed', False):
        return entry.get('passphrase', '').encode('utf-8')
    return None
...
class HBSYSTEM(DatagramProtocol):
    ...
    def master_datagramReceived(self, _data, _sockaddr):
        ...
        if _command == DMRD:
            ...
            # ---------------------------------------------------------------
            # PATCHED: per-device passphrase lookup from peer_auth.json
            # ---------------------------------------------------------------
            _peer_id = _data[4:8]
            if _peer_id in self._peers \
                ...
                _peer_id_int = int_id(_peer_id)
                _passphrase = _peer_passphrase(_peer_id_int)
                if _passphrase is None:
                    logger.warning('(%s) Peer %s denied: not found in peer_auth.json or not allowed', self._system, _peer_id_int)
                    self.transport.write(b''.join([MSTNAK, _peer_id]), _sockaddr)
                    del self._peers[_peer_id]
                else:
                    _calc_hash = bhex(sha256(_salt_str + _passphrase).hexdigest())
                    if _sent_hash == _calc_hash:
                        ...
                        logger.info('(%s) Peer %s has FAILED the login exchange', self._system, _this_peer['RADIO_ID'])
                        self.transport.write(b''.join([MSTNAK, _peer_id]), _sockaddr)
                        del self._peers[_peer_id]
            else:
                self.transport.write(b''.join([MSTNAK, _peer_id]), _sockaddr)
            ...
            _peer_id = _data[4:8]
            logger.info('(%s) Recieved DMR Talker Alias from peer %s', self._system, self._peers[_peer_id]['CALLSIGN'])
    ...
    def peer_datagramReceived(self, _data, _sockaddr):
        if self._config['MASTER_SOCKADDR'] == _sockaddr:
            ...
            if   _command == DMRD:
                _peer_id = _data[11:15]
                if self._config['LOOSE'] or _peer_id == self._config['RADIO_ID']:
                    _seq = _data[4:5]
                    _rf_src = _data[5:8]
                    _dst_id = _data[8:11]
                    _bits = _data[15]
                ...
                if self._stats['CONNECTION'] == 'RPTL_SENT':
                ...
                elif self._stats['CONNECTION'] == 'OPTIONS-SENT':
                    _peer_id = _data[6:10]
                    if self._config['LOOSE'] or _peer_id == self._config['RADIO_ID']:
...
class reportFactory(Factory):
    ...
    def send_config(self):
        serialized = pickle.dumps(self._config['SYSTEMS'], protocol=2)
...
if __name__ == '__main__':
    ...
    logger.info('(GLOBAL) HBlink \'HBlink.py\' (patched: per-device auth) -- SYSTEM STARTING...')
